<div class="container">
    <header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
        <a href="accueil" class="d-flex align-items-center col-md-3 mb-2 mb-md-0 text-dark text-decoration-none">
            <img src="public/assets/images/logo-menu.png" alt="logo de Charles Cantin" width="150" class="rounded">
        </a>

        <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
            <li><a href="galerie" class="nav-link px-2 link-dark">Galerie</a></li>
            <li><a href="tarifs" class="nav-link px-2 link-dark">Tarifs</a></li>
            <li><a href="contact" class="nav-link px-2 link-dark">Contact</a></li>
        </ul>

        <div class="col-md-3 text-end">
            <a href="connexion" class="btn btn-outline-dark me-2">
                <i class="fa-solid fa-user"></i>
            </a>
        </div>
    </header>
</div>

